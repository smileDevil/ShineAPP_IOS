...
class KJ70DataController: UIViewController {
    ...
    fileprivate let navTitleArr : [String] = ["实时数据","历史数据"]
...
extension KJ70DataController{
    func initView(){
        ...
        let titles = ["实时预警","实时报警","设备故障","测点信息","分站信息","开出口"]
        let style  = TJTitleStyle() 
               style.isScrollEnable = true
               style.isShowBottomLine = true
               style.isShowCover = false
        var childVcs = [UIViewController]()
        childVcs.append(EarlyWaringController())
        childVcs.append(WaringController())
        childVcs.append(EquipmentStateController())
        childVcs.append(TestPointController())
        childVcs.append(StationController())
        childVcs.append(OpOutController())
...
extension KJ70DataController:UIPickerViewDelegate,UIPickerViewDataSource {
